 ...
 * Tests are skipped gracefully when MySQL is unavailable.
 */
...
import { tryMySQL, standardSchema } from './helpers.js';

// Top-level check — skip all tests if MySQL is unavailable
const pool = await tryMySQL();

afterAll(async () => {
  if (pool) await pool.end();
});

describe.skipIf(!pool)('MySQL: Real Integration', () => {
  let db;

  beforeEach(async () => {
    const name = `mysql_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  afterEach(async () => {
    if (db) {
      ...
      for (const store of stores) {
        try { await pool.query(`DROP TABLE IF EXISTS \`${store}\``); } catch {}
      }
      try { await pool.query('DROP TABLE IF EXISTS `_easydb_meta`'); } catch {}
    }
  });
