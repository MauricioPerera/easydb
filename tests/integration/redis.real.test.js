...
import { tryRedis, standardSchema } from './helpers.js';

// Top-level check — skip all tests if Redis is unavailable
...
describe.skipIf(!redis)('Redis: Real Integration', () => {
  let db;
  let prefix;

  beforeEach(async () => {
    prefix = `test_${Date.now()}_${Math.random().toString(36).slice(2)}:`;
    db = await EasyDB.open(`redis_test`, {
      adapter: new RedisAdapter(redis, { prefix }),
  ...
  afterEach(async () => {
    if (db) {
      db.close();
      // Clean up keys with this prefix
