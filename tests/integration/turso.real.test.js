/**
 * Real Turso/libSQL Integration Tests
 *
 * Uses @libsql/client with in-memory database — no external service needed.
 * Tests are skipped only if @libsql/client is not installed.
 */
import { describe, it, expect, afterEach, beforeEach } from 'vitest';
import { EasyDB } from '../../src/easydb.js';
import { TursoAdapter } from '../../src/adapters/turso.js';
import { tryTurso, standardSchema } from './helpers.js';

// Top-level check — if @libsql/client is not available, all tests skip
const available = await tryTurso().then(c => { if (c) c.close(); return !!c; });

describe.skipIf(!available)('Turso: Real Integration', () => {
  let db;
  let client;

  beforeEach(async () => {
    // Each test gets a fresh in-memory client to avoid table conflicts
    const { createClient } = await import('@libsql/client');
    client = createClient({ url: 'file::memory:' });
    const name = `turso_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
