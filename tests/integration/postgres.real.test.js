...
import { tryPostgres, standardSchema } from './helpers.js';

// Top-level check — skip all tests if PostgreSQL is unavailable
const pool = await tryPostgres();

afterAll(async () => {
  if (pool) await pool.end();
});

describe.skipIf(!pool)('PostgreSQL: Real Integration', () => {
  let db;

  beforeEach(async () => {
    const name = `pg_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  afterEach(async () => {
    if (db) {
      ...
      for (const store of stores) {
        try { await pool.query(`DROP TABLE IF EXISTS "${store}" CASCADE`); } catch {}
      }
      try { await pool.query(`DROP TABLE IF EXISTS "_easydb_meta" CASCADE`); } catch {}
    }
  });
