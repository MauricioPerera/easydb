...
// Shared adapter instance so destroy() works
const adapter = new MemoryAdapter();
let dbCounter = 0;

async function createMemDB(customSchema) {
  const name = `mem-test-${Date.now()}-${dbCounter++}`;
...
describe('MemoryAdapter — CRUD', () => {
  ...
  it('data is cloned (mutation safety)', async () => {
    const original = { id: 1, name: 'Alice', email: 'a@t.com', role: 'admin', country: 'UY', age: 30 };
    await db.users.put(original);
    original.name = 'MUTATED';
    const stored = await db.users.get(1);
...
describe('MemoryAdapter — QueryBuilder', () => {
  let db, name;
...
describe('MemoryAdapter — Lifecycle', () => {
  ...
  it('reports version', async () => {
    const db = await EasyDB.open('mem-version-test', {
      adapter,
      version: 3,
  ...
  it('destroy removes database data', async () => {
    const db = await EasyDB.open('mem-destroy-test', {
      adapter,
      schema(s) { s.createStore('items', { key: 'id' }); }
    });
    ...
    await EasyDB.destroy('mem-destroy-test', { adapter });

    const db2 = await EasyDB.open('mem-destroy-test', {
      adapter,
      schema(s) { s.createStore('items', { key: 'id' }); }
    });
...
describe('MemoryAdapter — Unique indexes', () => {
...
// ── API parity with IDB adapter ──────────────────────────

describe('MemoryAdapter — API parity', () => {
  it('same query results as would be expected from IDB', async () => {
    const { db } = await createMemDB();
    await seedUsers(db, 10);
